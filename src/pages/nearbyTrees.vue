...
<script>
import { locationRef, geoFire } from '../plugins/firebase'
export default {
  name: 'nearby-trees',
  data () {
    return {
  ...
  methods: {
    ...
    callLocationsQuery () {
      this.getLocations(2, [this.lat, this.lng])
    },
    displayThumbnail (key) {
      let nearbytreeObj = this.$treesRef.child(key)
      nearbytreeObj.on('value', snapshot => {
        console.log('snapshot: ', snapshot.val())
        this.markerdisplay = snapshot.val().user_email || 'Anonymous'
        this.pictureDate = snapshot.val().datetime
        this.treeThumbnail = snapshot.val().s3url || 'Anonymous'
      })
    }
  },
  mounted () {
